...
// - Helius v1 path fix + resilient v0 fallback (prevents sol: null)import express from "express";
...
const { WebSocketServer } = ws;dotenv.config();
const app = express();
const PORT = process.env.PORT || 3000;app.use(cors());
app.use(express.json());function ts(){ const d=new Date(); return [${d.toTimeString().slice(0,8)}]; }
const log=(...a)=>console.log(ts(),...a);
const warn=(...a)=>console.warn(ts(),...a);
const err=(...a)=>console.error(ts(),...a);/* ---------- Supabase ---------- */
...
const supabase = createClient(SUPABASE_URL, SUPABASE_KEY);/* ---------- Health ---------- */
app.get("/healthz", (_req,res)=>res.json({ ok:true, time:new Date().toISOString() }));/* ---------- Chart poller (UNCHANGED) ---------- */
...
let memoryCache=[];async function insertPoint(point){
...
function getWindow(interval){ const now=Date.now(); if(interval==="D") return new Date(now-3086400e3).toISOString(); if(interval==="1h") return new Date(now-7*86400e3).toISOString(); return new Date(now-86400e3).toISOString(); }
...
const ICON_CACHE = new Map();async function fetchJSON(url, headers={}){
...
async function dsSearch(mint){
  ...
  if (cached && (now - cached.ts) < PRICE_TTL_MS) return cached;  const [ {price: dsP, changePct}, jPrice ] = await Promise.all([ dsPriceAndChange(mint), jupPrice(mint) ]);  const isMajor = MAJOR_SPL.has(mint);
  let price = 0, source = [];
  if (isMajor) {
  ...
  if (!price) { price = dsP || jPrice || 0; source = dsP ? ["dex"] : jPrice ? ["jup"] : []; }  const out = { priceUsd: price || 0, changePct: changePct || 0, confidence: pickConfidence(price||0), source, ts: now, raw: { jup: jPrice, dex: dsP } };
...
async function resolveTokenIcon(mint){
  if (!mint) return null;
  if (ICON_CACHE.has(mint)) return ICON_CACHE.get(mint);  try {
    ...
    if (img) { ICON_CACHE.set(mint, img); return img; }
  } catch {}  const gh = https://raw.githubusercontent.com/solana-labs/token-list/main/assets/mainnet/${mint}/logo.png;
...
const HELIUS_RPC = https://mainnet.helius-rpc.com/?api-key=${HELIUS_KEY};// v1: /v1/addresses/:address/balances (no tokenType filter; we'll keep all then filter to fungible if present)
async function fetchHeliusBalances(wallet){
  ...
  }catch(e){ warn("Helius v1 error:", e?.message||e); }  // Fallback v0
...
app.post("/api/balances", async (req, res) => {
 ...
 if (!wallet) return res.status(400).json({ error: "Missing wallet" });
 if (!HELIUS_KEY) return res.status(500).json({ error: "Backend missing HELIUS_API_KEY" }); // Pull balances
 const { json: data, version } = await fetchHeliusBalances(wallet); // SOL (normalize between v1 & v0)
 let lamports = 0;
 if (version === "v1") {
 ...
 const sol = (lamports || 0) / 1e9; // Tokens (v1: tokens[], v0: tokens[] or tokenBalances[])
 const rawTokens =
 ...
 logo: t.logo || t.image || t.info?.image || ""
   ...
   .filter(t => t.mint && t.uiAmount > 0); // SOL price + change
 const { priceUsd: solUsd, changePct: solChangePct } = await getSolUsdAndChange();
 const solUsdTotal = sol * solUsd; // Price tokens (BlackCoin override for name/symbol + price pref)
 const priced = await Promise.all(tokensBase.map(async t => {
   ...
   const icon   = await resolveTokenIcon(t.mint); // resolved icon   let priceUsd, changePct;
   if (isBlack) {
 ...
 changePct = Number(blend.changePct) || 0;
   }   const usd = priceUsd * t.uiAmount;   return {
 mint: t.mint,
 ...
 })); const tokens = priced.sort((a, b) => (b.usd || 0) - (a.usd || 0)); // ---- Portfolio Δ% with 25% per-asset cap ----
 const parts = [];
...
app.get("/api/price", async (req, res) => {
  try{
    ...
    if (!mint) return res.status(400).json({ error: "Missing mint" });if (mint === TOKEN_MINT) {
...
const clients = new Set();wss.on("connection", async (socket)=>{
  socket.isAlive = true;
  clients.add(socket);
  socket.on("pong", ()=> socket.isAlive=true);
  socket.on("close", ()=> clients.delete(socket));
  socket.on("error", (e)=> err("WS error:", e?.message||e));  // send last 25 on connect
  try{
    const { data, error } = await supabase
      .from("hub_broadcasts")
      .select("id, wallet, message, created_at")
  ...
  try {
    ...
    rtChannel = supabase
      ...
      .on("postgres_changes",
        ...
        (payload) => { const old = payload?.old || payload?.record || null; const id = old?.id; log("  DELETE hub_broadcasts id=", id); if (id) wsBroadcast({ type:"delete", id }); }
      )
      .subscribe((status) => {
        ...
        else if (status === "TIMED_OUT") { warn(" Realtime TIMED_OUT — retrying in 2s"); setTimeout(subscribeToBroadcasts, 2000); }
        else if (status === "CLOSED") { warn(" Realtime CLOSED — retrying in 2s"); setTimeout(subscribeToBroadcasts, 2000); }
      });
  } catch (e) {
...
subscribeToBroadcasts();server.listen(PORT, ()=> log( BlackCoin backend running on port ${PORT}));
