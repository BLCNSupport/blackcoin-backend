...
// Simple middleware: require that the session wallet is a DEV wallet
function requireDev(req, res, next) {
  const w = (req.sessionWallet || "").toLowerCase();
  if (!w || !DEV_WALLETS.has(w)) {
    warn("[admin] dev-only route blocked for wallet:", req.sessionWallet);
...
/* ======================================================================== */
/* =========================  ADMIN / DEV MONITORING  ===================== */
/* ======================================================================== */

/**
 * DEV-only aggregate stats for monitoring console.
 *
 * GET /api/admin/stats
 *   headers: { "x-bc-session": "<session token>" }
 *   -> 403 if not a DEV wallet
 */
app.get("/api/admin/stats", requireSession, requireDev, async (req, res) => {
  try {
    // 1) Pull stakes (we aggregate in Node)
    const { data: stakes, error: stakeErr } = await supabase
      .from("hub_stakes")
      .select("amount, status, is_void");

    if (stakeErr) {
      err("[admin/stats] hub_stakes error:", stakeErr.message);
      return res.status(500).json({ error: "stakes_query_failed" });
    }
    ...
    // 2) Aggregate FART rewards from claims
    const { data: claims, error: claimErr } = await supabase
      .from("hub_stake_claims")
      .select("amount");

    if (claimErr) {
      err("[admin/stats] hub_stake_claims error:", claimErr.message);
    }

    const totalFartClaims = (claims || []).length;
    const totalFartDistributed = (claims || []).reduce(
      (sum, c) => sum + Number(c.amount || 0),
      0
    );

    // 3) Pool balance snapshot (from hub_stake_pool)
    let poolInfo = null;
    try {
      const { data: poolRow, error: poolErr } = await supabase
        .from("hub_stake_pool")
        .select("wallet, fart_balance, updated_at")
        .eq("wallet", REWARD_POOL_PUBKEY)
        .maybeSingle();

      if (poolErr) {
        err("[admin/stats] hub_stake_pool error:", poolErr.message);
      } else if (poolRow) {
        poolInfo = {
          wallet: poolRow.wallet,
          fart_balance: Number(poolRow.fart_balance || 0),
          updated_at: poolRow.updated_at,
        };
      }
    } catch (e) {
      err("[admin/stats] pool select exception:", e?.message || e);
    }

    // 4) Recent burns for quick visibility
    let recentBurns = [];
    try {
      const { data: burnRows, error: burnsErr } = await supabase
        .from("hub_burns")
        .select("wallet, token, amount, timestamp, signature")
        .order("timestamp", { ascending: false })
        .limit(10);

      if (burnsErr) {
        err("[admin/stats] hub_burns error:", burnsErr.message);
      } else {
        recentBurns = (burnRows || []).map((b) => ({
          wallet: b.wallet,
          token: b.token,
      ...
      err("[admin/stats] burns select exception:", e?.message || e);
    }
    ...
    return res.json({
      ...
      claims: {
        totalFartClaims,
        totalFartDistributed,
      },
      pool: poolInfo,
      burns: recentBurns,
    });
  } catch (e) {
    err("[admin/stats] exception:", e?.message || e);
    return res.status(500).json({ error: "admin_stats_failed" });
  }
});

/**
 * DEV-only list of stakes (paginated / filterable by wallet).
 *
 * GET /api/admin/stakes?wallet=<optional>&limit=50
 */
app.get("/api/admin/stakes", requireSession, requireDev, async (req, res) => {
  try {
    const walletFilter = String(req.query.wallet || "").trim();
    const limit = Math.min(parseInt(req.query.limit) || 50, 200);
    ...
    if (error) {
      err("[admin/stakes] select error:", error.message);
      return res.status(500).json({ error: "stakes_query_failed" });
    }
    ...
    err("[admin/stakes] exception:", e?.message || e);
    return res.status(500).json({ error: "admin_stakes_failed" });
  }
});

/**
 * DEV-only detailed health for backend:
 * - memory usage
 * - number of WS clients
 */
app.get("/api/admin/health", requireSession, requireDev, async (_req, res) => {
  try {
    ...
    err("[admin/health] exception:", e?.message || e);
    res.status(500).json({ error: "admin_health_failed" });
  }
});
